...
public class TowerHandler
{
	...
	if (keyHandler.getClickPoint() != null) {
		...
		for (int j = 0; j < grid.getRectangles()[i].length; j++) {
			...
			if (grid.getRectangles()[i][j].contains(keyHandler.getClickPoint()))
				...
				tower.setRange();
				tower.setRectangle(new Rectangle(tower.getX(), tower.getY(), tower.getImage().getWidth(null),
								 tower.getImage().getHeight(null)));
	...
	for (int i = 0; i < towers.size(); i++) {
		...
		if (towers.get(i).getRectangle().contains(keyHandler.getClickPoint())) {
		    ...
		    upgradeTowerRange =
			    new Rectangle(UPGRADEWIDTHCOORD, grid.getHeight() + UPGRADEHEIGHTCORRECTIONCOORD, 100, 100);
		    upgradeTowerDamage =
			    new Rectangle(UPGRADEWIDTHCOORD * 2, grid.getHeight() + UPGRADEHEIGHTCORRECTIONCOORD, 100, 100);
	...
	for (Towers tower : towers) {
		...
		g.drawString("Upgrades: " + tower.getUpgrades() + "/" + MAX_UPGRADES, UPGRADEWIDTHCOORD * 3 + 100,
			     grid.getHeight() + UPGRADEHEIGHTCORRECTIONCOORD + 20);
		...
		g.drawString("$" + tower.getUpgradeRangeCost(), UPGRADEWIDTHCOORD,
			     grid.getHeight() + UPGRADEHEIGHTCORRECTIONCOORD + 10);

		g.drawString("$" + tower.getUpgradeDamageCost(), UPGRADEWIDTHCOORD * 2,
			     grid.getHeight() + UPGRADEHEIGHTCORRECTIONCOORD + 10);

		g.drawString("$" + tower.getSell(), UPGRADEWIDTHCOORD * 3,
			     grid.getHeight() + UPGRADEHEIGHTCORRECTIONCOORD + 10);
